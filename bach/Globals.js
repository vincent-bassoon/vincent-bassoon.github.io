...
class HarmonyUnit {
	constructor(target_avgs, is_end_of_phrase){
		this.values = [[null, null, null, null], [null, null, null, null], [null, null, null, null]];
		this.leap = [null, null, null, null];
		
		this.chord = null;
		
		this.is_end_of_phrase = is_end_of_phrase;
		
		this.score_unit = new ScoreUnit(target_avgs, this);
	}
	getNumNotes(voice){
...
class NoteFunctions {
	constructor(){
		...
		this.preferred_ranges = {0: {"min": 5 + 24, "max": 4 + 36},
					 1: {"min": 5 + 36, "max": 4 + 48},
					 2: {"min": 9 + 36, "max": 8 + 48},
					 3: {"min": 4 + 48, "max": 3 + 60}},
	
		this.absolute_ranges = {0: {"min": 0 + 24, "max": 0 + 48},
					1: {"min": 0 + 36, "max": 7 + 48},
					2: {"min": 7 + 36, "max": 2 + 60},
					3: {"min": 0 + 48, "max": 7 + 60}};
	}
