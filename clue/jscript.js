...
function getHistory(){
	firebase.database().ref("j/history").once('value').then(function(snapshot){
...
function configureEdit(){
	...
	document.getElementById("submit").onclick = function(){
		...
		if(actions.status == null && actions.edit == null && actions.add == null){
			...
			firebase.database().ref('j/history/' + (change_history.length - 1)).set(change_history[change_history.length - 1]);
		}
		clearBoxes();
		document.getElementById("input-container").style.display = "none";
		document.getElementById("main-container").style.display = "block";
	...
	function reset(){
		clearData();
		change_history = [];
		firebase.database().ref('j/history').set(change_history);
		document.getElementById("confirm-container").style.display = "none";
		document.getElementById("input-container").style.display = "none";
		document.getElementById("main-container").style.display = "block";
	}
	...
	function undo(){
		change_history.pop();
		firebase.database().ref('j/history/' + (change_history.length - 1)).remove();
		change_history.pop();
		clearData();
		for(var i = 0; i < change_history.length; i++){
			execute(change_history[i].a, change_history[i].s);
